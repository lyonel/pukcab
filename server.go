...
func submitfiles() {
	...
	if err := catalog.QueryRow("SELECT COUNT(*) FROM files WHERE backupid=? AND type='?'", date).Scan(&missing); err == nil {
		if missing == 0 {
			...
			log.Printf("Finished backup: date=%d name=%q schedule=%q files=%d received=%d\n", date, name, schedule, files, received)
			fmt.Printf("Backup %d complete (%d files)\n", date, files)
		} else {
			log.Printf("Received files for backup set: date=%d name=%q schedule=%q files=%d missing=%d received=%d\n", date, name, schedule, files, missing, received)
