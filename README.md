...
1. [https://en.wikipedia.org/wiki/Ssh-keygen create SSH keys] for the user which will launch the backup (most probably `root`)
...
1. [create SSH keys](https://en.wikipedia.org/wiki/Ssh-keygen) for the user which will launch the backup (most probably `root`)
1. [#register register] to the backup server
 ...
 * the `vault` folder __must not be used to store anything__ else than `pukcab`'s data files; in particular, do **NOT** store the `catalog` there
...
| `include` | list of text values | cf. below | specifies filesystems/folders/files to include in the backup |
| `exclude` | list of text values | cf. below | specifies filesystems/folders/files to exclude from the backup |
...
## Usage
### Synopsis
`pukcab` [_COMMAND](#commands) [[_OPTIONS](#options)...] [[_FILES](#files)...]

#### Commands
| backup and recovery | |
|[backup](#backup)[[br]][save](#backup) | take a new backup |
|[continue](#continue)[[br]][resume](#continue) | continue a partial backup |
|[restore](#restore) | restore files |
|[verify](#verify)[[br]][check](#verify) | verify files in a backup |
|= maintenance =|
|[delete](#delete)[[br]][purge](#delete) | delete a backup |
|[expire](#expire) | apply retention schedule to old backups |
|= utilities =|
|[info](#info)[[br]][list](#info) | list backups and files |
|[ping](#ping)[[br]][test](#ping) | check server connectivity |
|[register](#register) | register to backup server |
...
Syntax
  `pukcab backup` [ --[#full full] ] [ --[#name name]=_name ] [ --[#schedule schedule]=schedule ]

Note
  the [#name name] and [#schedule schedule] options are chosen automatically if not specified

### continue
This command continues a previously interrupted backup.

Syntax
  `pukcab continue` [ --[#name name]=*name* ] [ --[#date date]=*date* ]

Notes
 * the [#name name] option is chosen automatically if not specified
 * the [#date date] option automatically selects the last unfinished backup
 * only unfinished backups may be resumed

### restore
This command restores [#files files] as they were at a given [#date date].

Syntax
  `pukcab restore` [ --[#name name]=*name* ] [ --[#date date]=*date* ] [ *[#files FILES]*... ]

Notes
 * the [#name name] option is chosen automatically if not specified
 * the [#date date] option automatically selects the last backup
 * this operation currently requires a working `tar` system command (usually GNU tar)
 
### verify
This command reports [#files files] which have changed since a given [#date date].

Syntax
  `pukcab verify` [ --[#name name]=*name* ] [ --[#date date]=*date* ] [ *[#files FILES]*... ]

Notes
 * the [#name name] option is chosen automatically if not specified
 * the [#date date] option automatically selects the last backup if not specified
 
### delete
This command discards the backup taken at a given [#date date].

Syntax
  `pukcab delete` [ --[#name name]=*name* ] --[#date date]=*date*

Notes
 * the [#name name] option is chosen automatically if not specified
 * the [#date date] must be specified
 
### expire
This command discards backups following a given [#schedule schedule] which are older than a given [#date age (or date)]. Standard retention schedules have pre-defined retention periods:
...
Syntax
  `pukcab expire` [ --[#name name]=*name* ] [ --[#schedule schedule]=*schedule* ] [ --[#date age]=*age* ] [ --[#keep keep]=*keep* ]

Notes
 * on the [#Server backup server], the [#name name] option defaults to all backups if not specified
 * on a [#Client backup client], the [#name name] option is chosen automatically if not specified
 * the [#schedule schedule] and [#date expiration] are chosen automatically if not specified
 
### info
This command lists the backup sets stored on the server. Backup sets can be filtered by name and/or date and files.

Syntax
  `pukcab info` [ --[#name name]=*name* ] [ --[#date date]=*date* ] [ *[#files FILES]*... ]

Notes
 * if [#date date] is specified, the command lists only details about the corresponding backup
 * if [#name name] is not specified, the command lists all backups, regardless of their name
 * verbose mode lists the individual [#files files]

### ping
This command allow to check connectivity to the server.
...
This means that the following lines are equivalent:
 * `pukcab info -n test`
 * `pukcab info -n=test`
 * `pukcab info --n test`
 * `pukcab info --n=test`
...
| option | description |
|--------|-------------|
|`-c`, `--config`[=]*file* | specify a [#Configuration configuration file] to use |
...
All backup sets are identified by a unique numeric id and correspond to a set of files at a given point in time (the backup id is actually a [https://en.wikipedia.org/wiki/Unix_time UNIX timestamp]).
...
Examples
  `--date 1422577319` means *on the 30th January 2015 at 01:21:59 CET*[[br]]
  `--date 0`, `--date now` and `--date latest` mean *now*[[br]]
  `--date today` means *today at 00:00*[[br]]
  `--date 1` means *yesterday same time*[[br]]
  `--date 7` means *last week*[[br]]
  `--date -48h` and `--date 48h` both mean *2 days ago*[[br]]
  `--date 2h30m` means *2 hours and 30 minutes ago*[[br]]
...
Examples
  `/home` includes `/home/john`, `/home/dave`, etc. and all the files they contain (i.e. all users' home directories)[[br]]
  `*.jpg` includes all `.jpg` files (JPEG images) in all directories[[br]]
